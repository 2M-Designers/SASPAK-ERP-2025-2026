...
type LineItemApproval = {
  internalFundsRequestBankId: number; // Bank PK (not cash)
  jobId: number | null;
  jobNumber: string;
  headCoaId: number;
  ...
  beneficiary: string;
  accountNo: string; // Bank-specific field
  ...
  createdOn: string;
  version?: number;
  bankFundRequestMasterId?: number; // Bank FK (not cash)
};
...
export default function InternalBankFundRequestApprovalForm({
  ...
  // ── Fetch jobs to resolve job numbers ─────────────────────────────────────
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const items: LineItemApproval[] = requestData.internalBankFundsRequests.map(
      (detail: any) => {
        ...
        // Resolve job number: embedded object → jobs list → raw id
        const jobObj =
    ...
    console.log("✅ Initialized bank approval line items:", items);
    setLineItems(items);
  }, [requestData, jobs]);
  ...
  const submitApproval = async (status: "APPROVED" | "REJECTED") => {
    ...
    console.log(`BANK APPROVAL UPDATE: ${status}`);
    ...
    try {
      ...
      // bankId at MASTER level (not jobId — Bank schema has bankId here)
      const payload = {
        ...
        // Navigation nulls required by API
      ...
      console.log("📦 BANK APPROVAL PAYLOAD:");
      ...
      toast({
        title: "Success",
        description: `Bank fund request ${status.toLowerCase()} successfully. ${
          status === "APPROVED"
      ...
      toast({
        variant: "destructive",
        title: "Error",
        description: `Failed to ${status.toLowerCase()} bank fund request: ${
      ...
      <Card>
        <CardHeader className='py-3 px-4 bg-purple-50'>
          ...
          <CardDescription className='pt-2'>
            Review and approve/reject the bank fund request. You can modify
            approved amounts for each line item.
          </CardDescription>
        </CardHeader>
                    ...
                    {lineItems.map((item, index) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {/* Account No — read-only in approval */}
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className='space-y-2'>
              <p>Are you sure you want to approve this bank fund request?</p>
      ...
      <AlertDialog open={showRejectDialog} onOpenChange={setShowRejectDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className='space-y-2'>
              <p>Are you sure you want to reject this bank fund request?</p>
              <div className='mt-3 p-3 bg-red-50 border border-red-200 rounded'>
                <p className='text-sm font-semibold text-red-900'>
                  Request ID: #{requestData?.bankFundRequestId}
                </p>
                <p className='text-xs text-red-700 mt-1'>
