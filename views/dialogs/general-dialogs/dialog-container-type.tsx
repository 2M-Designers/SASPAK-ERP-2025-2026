"use client";

import { Button } from "@/components/ui/button";
import {
  Dialog,
  DialogContent,
  DialogDescription,
  DialogHeader,
  DialogTitle,
  DialogTrigger,
} from "@/components/ui/dialog";
import {
  Form,
  FormControl,
  FormField,
  FormItem,
  FormLabel,
  FormMessage,
} from "@/components/ui/form";
import { Input } from "@/components/ui/input";
import { Textarea } from "@/components/ui/textarea";
import { useToast } from "@/hooks/use-toast";
import { Loader2, Plus } from "lucide-react";
import { useState, useEffect } from "react";
import { useForm } from "react-hook-form";
import { zodResolver } from "@hookform/resolvers/zod";
import * as z from "zod";

// Validation Schema - updated to match API
const formSchema = z.object({
  containerTypeId: z.number().optional(),
  typeCode: z.string().min(1, "Type Code is required"),
  typeName: z.string().min(1, "Type Name is required"),
  isHazardousAllowed: z.boolean().default(false),
  isActive: z.boolean().default(true),
  remarks: z.string().optional(),
  companyId: z.number().optional(),
  version: z.number().optional(),
});

interface ContainerTypeDialogProps {
  type: "add" | "edit";
  defaultState: any;
  handleAddEdit: (item: any) => void;
  children?: React.ReactNode;
}

export default function ContainerTypeDialog({
  type,
  defaultState,
  handleAddEdit,
  children,
}: ContainerTypeDialogProps) {
  const { toast } = useToast();
  const [open, setOpen] = useState(false);
  const [isLoading, setIsLoading] = useState(false);

  const form = useForm<z.infer<typeof formSchema>>({
    resolver: zodResolver(formSchema),
    defaultValues: {
      containerTypeId: defaultState.containerTypeId || undefined,
      typeCode: defaultState.typeCode || "",
      typeName: defaultState.typeName || "",
      isHazardousAllowed:
        defaultState.isHazardousAllowed !== undefined
          ? defaultState.isHazardousAllowed
          : false,
      isActive:
        defaultState.isActive !== undefined ? defaultState.isActive : true,
      remarks: defaultState.remarks || "",
      companyId: defaultState.companyId || 0,
      version: defaultState.version || 0,
    },
  });

  // Reset form when dialog opens/closes or defaultState changes
  useEffect(() => {
    if (open) {
      // Get company ID from localStorage or default to 0
      const user = localStorage.getItem("user");
      let companyId = 1;

      if (user) {
        try {
          const u = JSON.parse(user);
          companyId = u?.companyId || 1;
        } catch (error) {
          console.error("Error parsing user JSON:", error);
        }
      }

      form.reset({
        containerTypeId: defaultState.containerTypeId || undefined,
        typeCode: defaultState.typeCode || "",
        typeName: defaultState.typeName || "",
        isHazardousAllowed:
          defaultState.isHazardousAllowed !== undefined
            ? Boolean(defaultState.isHazardousAllowed)
            : false,
        isActive:
          defaultState.isActive !== undefined
            ? Boolean(defaultState.isActive)
            : true,
        remarks: defaultState.remarks || "",
        companyId:
          type === "add" ? companyId : defaultState.companyId || companyId,
        version: defaultState.version || 0,
      });
    }
  }, [open, type, defaultState, form]);

  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    const user = localStorage.getItem("user");
    let userID = 0;
    let companyId = 1;
    let userName = "";

    if (user) {
      try {
        const u = JSON.parse(user);
        userID = u?.userID || 0;
        companyId = u?.companyId || 1;
        userName = u?.userName || u?.email || "";
      } catch (error) {
        console.error("Error parsing user JSON:", error);
      }
    }

    const isUpdate = type === "edit";

    // Prepare payload according to API requirements
    const payload: any = {
      typeCode: values.typeCode,
      typeName: values.typeName,
      isHazardousAllowed: values.isHazardousAllowed,
      isActive: values.isActive,
      remarks: values.remarks || "",
      version: values.version || 0,
    };

    // For add operation
    if (!isUpdate) {
      payload.companyId = companyId; // From localStorage
    } else {
      // For edit operation - include the ID and companyId
      payload.containerTypeId = values.containerTypeId;
      payload.companyId = values.companyId;
      // Increment version for edit
      payload.version = values.version || 0;
    }

    // Add user tracking and timestamps if needed
    // const now = new Date().toISOString();
    // if (isUpdate) {
    //   payload.updatedAt = now;
    //   payload.updatedBy = userID;
    // } else {
    //   payload.createdAt = now;
    //   payload.createdBy = userID;
    // }

    console.log("Container Type Payload:", payload);
    setIsLoading(true);

    try {
      const response = await fetch(
        process.env.NEXT_PUBLIC_BASE_URL + "SetupContainerType",
        {
          method: isUpdate ? "PUT" : "POST",
          headers: {
            "Content-Type": "application/json",
          },
          body: JSON.stringify(payload),
        }
      );

      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.message || `HTTP Error: ${response.status}`);
      }

      const responseData = await response.text();
      const jsonData = responseData ? JSON.parse(responseData) : null;

      if (jsonData?.statusCode >= 400) {
        throw new Error(jsonData.message || "Unknown error occurred");
      }

      setOpen(false);

      // Prepare the response data to include in handleAddEdit
      const responseItem = {
        ...values,
        ...jsonData,
        // For edit operations, use incremented version
        ...(isUpdate ? { version: payload.version } : {}),
      };

      handleAddEdit(responseItem);

      toast({
        title: `Container type ${
          type === "edit" ? "updated" : "added"
        } successfully.`,
      });
    } catch (error: any) {
      console.error("API Error Details:", error);
      toast({
        variant: "destructive",
        title: "Error",
        description:
          error.message || "Failed to process container type operation",
      });
    } finally {
      setIsLoading(false);
    }
  };

  const handleOpenChange = (open: boolean) => {
    setOpen(open);
  };

  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogTrigger asChild>
        {type === "edit" ? (
          children || <Button variant='outline'>Edit</Button>
        ) : (
          <Button className='flex items-center gap-2'>
            <Plus size={16} />
            Add Container Type
          </Button>
        )}
      </DialogTrigger>
      <DialogContent className='max-w-2xl max-h-[90vh] overflow-y-auto'>
        <DialogHeader>
          <DialogTitle>
            {type === "edit" ? "Edit Container Type" : "Add New Container Type"}
          </DialogTitle>
          <DialogDescription>
            {type === "edit"
              ? "Update container type information."
              : "Add a new container type to the system."}
          </DialogDescription>
        </DialogHeader>

        <Form {...form}>
          <form
            onSubmit={form.handleSubmit(onSubmit)}
            className='flex flex-col gap-4'
          >
            {/* Display Company Info (read-only) */}
            <div className='p-3 bg-gray-50 rounded-md'>
              <div>
                <FormLabel className='text-sm font-medium text-gray-600'>
                  Company
                </FormLabel>
                <div className='mt-1 text-sm text-gray-900'>
                  {(() => {
                    const user = localStorage.getItem("user");
                    if (user) {
                      try {
                        const u = JSON.parse(user);
                        return u?.companyName || "SASPAK CARGO";
                      } catch (error) {
                        return "SASPAK CARGO";
                      }
                    }
                    return "SASPAK CARGO";
                  })()}
                </div>
              </div>
            </div>

            <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
              {/* Type Code */}
              <FormField
                control={form.control}
                name='typeCode'
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>Type Code *</FormLabel>
                    <FormControl>
                      <Input
                        placeholder='Enter type code (e.g., DRY, REEF, TANK)'
                        {...field}
                        className='uppercase'
                        onChange={(e) =>
                          field.onChange(e.target.value.toUpperCase())
                        }
                      />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              />

              {/* Type Name */}
              <FormField
                control={form.control}
                name='typeName'
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>Type Name *</FormLabel>
                    <FormControl>
                      <Input
                        placeholder='Enter type name (e.g., Dry Container, Tank Container)'
                        {...field}
                      />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              />
            </div>

            <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
              {/* Is Hazardous Allowed */}
              <FormField
                control={form.control}
                name='isHazardousAllowed'
                render={({ field }) => (
                  <FormItem className='flex flex-row items-start space-x-3 space-y-0 rounded-md border p-4'>
                    <FormControl>
                      <input
                        type='checkbox'
                        checked={field.value}
                        onChange={field.onChange}
                        className='w-4 h-4 mt-1 rounded border-gray-300 text-blue-600 focus:ring-blue-500'
                      />
                    </FormControl>
                    <div className='space-y-1 leading-none'>
                      <FormLabel>Hazardous Allowed</FormLabel>
                      <p className='text-sm text-muted-foreground'>
                        Allow hazardous materials in this container type
                      </p>
                    </div>
                    <FormMessage />
                  </FormItem>
                )}
              />

              {/* Is Active */}
              <FormField
                control={form.control}
                name='isActive'
                render={({ field }) => (
                  <FormItem className='flex flex-row items-start space-x-3 space-y-0 rounded-md border p-4'>
                    <FormControl>
                      <input
                        type='checkbox'
                        checked={field.value}
                        onChange={field.onChange}
                        className='w-4 h-4 mt-1 rounded border-gray-300 text-blue-600 focus:ring-blue-500'
                      />
                    </FormControl>
                    <div className='space-y-1 leading-none'>
                      <FormLabel>Status</FormLabel>
                      <p className='text-sm text-muted-foreground'>
                        {field.value ? "Active" : "Inactive"}
                      </p>
                    </div>
                    <FormMessage />
                  </FormItem>
                )}
              />
            </div>

            {/* Remarks */}
            <FormField
              control={form.control}
              name='remarks'
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Remarks</FormLabel>
                  <FormControl>
                    <Textarea
                      placeholder='Enter any remarks about this container type...'
                      {...field}
                      rows={3}
                    />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
            />

            {/* Hidden fields for version tracking and IDs */}
            <div className='hidden'>
              <FormField
                control={form.control}
                name='version'
                render={({ field }) => (
                  <FormItem>
                    <FormControl>
                      <Input type='hidden' {...field} />
                    </FormControl>
                  </FormItem>
                )}
              />
              <FormField
                control={form.control}
                name='companyId'
                render={({ field }) => (
                  <FormItem>
                    <FormControl>
                      <Input type='hidden' {...field} />
                    </FormControl>
                  </FormItem>
                )}
              />
              {type === "edit" && (
                <FormField
                  control={form.control}
                  name='containerTypeId'
                  render={({ field }) => (
                    <FormItem>
                      <FormControl>
                        <Input type='hidden' {...field} />
                      </FormControl>
                    </FormItem>
                  )}
                />
              )}
            </div>

            {/* Version info for edit mode 
            {type === "edit" && (
              <div className='p-3 bg-blue-50 rounded-md'>
                <div className='flex items-center justify-between'>
                  <div>
                    <FormLabel className='text-sm font-medium text-blue-600'>
                      Version Information
                    </FormLabel>
                    <p className='text-sm text-blue-700'>
                      Current version: {form.watch("version") || 0}
                    </p>
                  </div>
                  <div className='text-sm text-blue-600'>
                    <p>Version will be incremented on update</p>
                  </div>
                </div>
              </div>
            )}*/}

            <div className='flex justify-end gap-3 pt-4'>
              <Button
                type='button'
                variant='outline'
                onClick={() => setOpen(false)}
                disabled={isLoading}
              >
                Cancel
              </Button>
              <Button type='submit' disabled={isLoading}>
                {isLoading && <Loader2 className='mr-2 h-4 w-4 animate-spin' />}
                {type === "edit"
                  ? "Update Container Type"
                  : "Add Container Type"}
              </Button>
            </div>
          </form>
        </Form>
      </DialogContent>
    </Dialog>
  );
}
