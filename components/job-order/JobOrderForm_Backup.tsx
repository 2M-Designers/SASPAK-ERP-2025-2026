...
export default function JobOrderForm({
  ...
  // Tracks real form mode — starts from prop but can flip "add" → "edit"
  // after a job is created and the user chooses to stay on the page.
  const [internalType, setInternalType] = useState<"add" | "edit">(type);
      ...
      const loadedInvoices = jobData.jobInvoices.map((invoice: any) => {
        ...
        const items = (invoice.jobInvoiceCommodities || []).map((item: any) => {
          ...
          return {
            ...
            totalValue: item.totalValueDv || 0,
        ...
        const loadedInvoice = {
          ...
          fiDate: invoice.flDate ? invoice.flDate.split("T")[0] : "",
          fiExpiryDate: invoice.expiryDate
            ? invoice.expiryDate.split("T")[0]
            : "",
    ...
    if (
      ...
      const transformedDispatch = jobData.jobEquipmentHandingOvers.map(
        (dispatch: any) => ({
          ...
          containerReturnTerminalId: dispatch.containerReturnTerminalId || 0,
    ...
    if (
      ...
      const transformedDetention = jobData.jobEquipmentDetentionDetails.map(
        (detention: any) => ({
          ...
          damage: detention.damage || "",
          dirty: detention.dirty || "",
  ...
  useEffect(() => {
    const fetchJobData = async () => {
      // ✅ Use internalType so this also re-runs after "add" flips to "edit"
      if (internalType === "edit" && defaultState?.jobId) {
        try {
  ...
  const handleFormSubmit = (values: any) => {
    const confirmed = window.confirm(
      `⚠️ CONFIRM ${internalType === "edit" ? "UPDATE" : "CREATE"}\n\n` +
        `Are you sure you want to ${internalType === "edit" ? "update" : "create"} this job order?\n\n` +
  ...
  const onSubmit = async (values: any) => {
    try {
      ...
      if (internalType === "edit" && values.jobId) {
      ...
      const jobEquipments = fclContainers.map((container) => {
        ...
        const latestEquipmentVersion =
          internalType === "edit" && equipmentId > 0
      ...
      const jobCommoditiesPayload = jobCommodities.map((commodity) => {
        ...
        const latestCommodityVersion =
          internalType === "edit" && commodityId > 0
      ...
      if (goodsDeclarations.length > 0) {
        jobGoodsDeclarations = goodsDeclarations.map((gd: any) => {
          ...
          const latestGdVersion =
            internalType === "edit" && actualGdId > 0
          ...
          return {
            JobGoodsDeclarationId: actualGdId,
      ...
      const jobEquipmentHandingOvers = dispatchRecords.map((dispatch: any) => {
        ...
        const latestDispatchVersion =
          internalType === "edit" && dispatchId > 0
      ...
      const jobEquipmentDetentionDetails = detentionRecords.map(
        (detention: any) => {
          ...
          const latestDetentionVersion =
            internalType === "edit" && detentionId > 0
      ...
      const payload: any = {
        ...
        ...(internalType === "add" || jobGoodsDeclarations.length > 0
      ...
      // ─────────────────────────────────────────────────────────────
      // ✅ POST-SAVE BEHAVIOUR
      // After creating a new job, ask if the user wants to stay and
      // continue editing (switching seamlessly into Edit mode), or
      // navigate back to the list.
      // ─────────────────────────────────────────────────────────────
      if (internalType === "add") {
        const stayOnPage = window.confirm(
          "✅ Job Created Successfully!\n\n" +
            `Job Number: ${result.jobNumber || "N/A"}\n\n` +
            "Would you like to stay on this page and continue editing?\n\n" +
            "• OK     → Stay and keep editing (Edit mode)\n" +
            "• Cancel → Return to the jobs list",
        );

        if (stayOnPage) {
          // Flip the form into Edit mode with the freshly created data.
          // populateForm resets the main form + all child record states.
          setInternalType("edit");
          populateForm(result);
          toast({
            title: "Job Created!",
            description:
              "Switched to Edit mode — you can continue updating this job.",
          });
          // Do NOT call handleAddEdit — we intentionally stay on the page.
        } else {
          toast({
            title: "Success!",
        ...
        // Normal edit save — always navigate away as before.
        toast({
          title: "Success!",
