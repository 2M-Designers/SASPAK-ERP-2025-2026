  ...
  const fetchFundRequests = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${baseUrl}InternalFundsRequestCash/GetList`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            select:
              "InternalFundsRequestCashId, JobId, HeadCoaId, BeneficiaryCoaId, HeadOfAccount, Beneficiary, RequestedAmount, ApprovedAmount, ApprovalStatus, ApprovedBy, ApprovedOn, RequestedTo, CreatedOn, CreatedBy, Version",
            where: "",
  ...
  const handleDelete = async (item: InternalFundRequest) => {
    if (
      ...
      try {
        const baseUrl = process.env.NEXT_PUBLIC_BASE_URL;
        const response = await fetch(
          `${baseUrl}InternalFundsRequestCash/${item.internalFundsRequestCashId}`,
          {
            method: "DELETE",
          },
        );
