...
export default function ClientComponent({ initialData }: HomePageClientProps) {
  ...
  const fetchFilteredData = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL}Utility/GetAllNotifications`
      );
      if (response.ok) {
  ...
  const handleRefresh = () => {
    ...
    router.push("/portal/reports/notifications");
  };
  ...
  const columns: ColumnDef<any>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => row.getValue("notificationType"),
      enableColumnFilter: false,
    },
    {
      accessorKey: "message",
    ...
    },
    {
      ...
      header: "Entered On",
      cell: ({ row }) =>
        moment(row.getValue("enteredOn")).format("YYYY-MM-DD HH:mm A"), // Date + Time Format
      enableColumnFilter: false,
    },
  ];
