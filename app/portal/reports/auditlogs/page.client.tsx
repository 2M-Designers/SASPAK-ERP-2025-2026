...
export default function ClientComponent({ initialData }: HomePageClientProps) {
  ...
  const fetchFilteredData = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL}Utility/GetAllAuditLogs`
      );
      if (response.ok) {
  ...
  const handleRefresh = () => {
    ...
    router.push("/portal/reports/auditlogs");
  };
  ...
  const columns: ColumnDef<any>[] = [
    ...
    },
    {
      ...
      header: "Entered On",
      cell: ({ row }) =>
        moment(row.getValue("timeStamp")).format("YYYY-MM-DD HH:mm A"), // Date + Time Format
      enableColumnFilter: false,
    },
  ];
